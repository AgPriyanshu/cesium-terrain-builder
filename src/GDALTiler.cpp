...
GDALTiler::GDALTiler(GDALDataset *poDataset, const Grid &grid, const TilerOptions &options) : mGrid(grid),
																																															poDataset(poDataset),
																																															options(options)
{
...
GDALTiler::GDALTiler(const GDALTiler &other) : mGrid(other.mGrid),
...
GDALTiler::GDALTiler(GDALTiler &other) : mGrid(other.mGrid),
																				 poDataset(other.poDataset),
																				 mBounds(other.mBounds),
																				 mResolution(other.mResolution),
																				 crsWKT(other.crsWKT)
	...
	if (nOvCount > 0)
	{
																 ...
																 adfExtent, 0) == CE_None)
		{
			...
			if (dfTargetRatio > 1.0)
			{
				...
				{
					double dfOvrRatio = (iOvr < 0) ? 1.0 : (double)poSrcDS->GetRasterXSize() / poSrcDS->GetRasterBand(1)->GetOverview(iOvr)->GetXSize();
...
GDALTiler::createRasterTile(double (&adfGeoTransform)[6]) const
{
	...
	for (short unsigned int i = 0; i < psWarpOptions->nBandCount; ++i)
	{
		...
		psWarpOptions->padfDstNoDataReal[i] = new_noDataValue;
